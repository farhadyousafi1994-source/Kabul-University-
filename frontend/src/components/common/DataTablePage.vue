  ...
  <div class="page-container q-pa-md q-pa-lg-md">
    ...
    <div class="row items-center q-col-gutter-sm q-mb-sm">
      ...
      <div class="col-6 col-md-2">
        <q-btn label="Reset" outline dense color="grey-7" :disable="!hasActiveFilters" @click="resetFilters" />
      </div>
    </div>
    ...
    <template v-else>
      ...
      >
        <template v-slot:body-cell-actions="props">
          <q-td :props="props">
            ...
            <q-btn
              ...
              :aria-label="`Edit ${entityLabel}`" data-cy="edit-btn"
            ...
            <q-btn
              ...
              :aria-label="`Delete ${entityLabel}`" data-cy="delete-btn"
        ...
        <template v-if="isEmpty" v-slot:no-data>
          <EmptyState
            ...
            :action-label="createForm && canCreate ? createLabel : ''"
            @action="openCreate"
          />
        </template>
      </q-table>
      ...
      <div class="row items-center justify-between q-mt-md q-gutter-sm">
        <div class="text-caption text-grey-6">
          Showing {{ rows.length }} of {{ total }} · Page {{ page }} of {{ lastPage }}
        </div>
        <q-pagination
          v-model="page"
    ...
    <q-dialog v-model="dialogOpen" persistent>
      <q-card style="min-width: 420px; max-width: 720px" class="q-dialog-card">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">{{ editing ? `Edit ${entityLabel}` : createLabel }}</div>
          <q-space />
          <q-btn flat round dense icon="close" aria-label="Close" @click="dialogOpen = false" />
        </q-card-section>
          ...
          <q-form @submit="save" class="row q-col-gutter-md">
            ...
            >
              <q-input
                ...
                dense
                outlined
                :rules="[(v) => !field.required || (v !== null && v !== undefined && String(v).trim() !== '') || `${field.label} is required`]"
              />
              <q-select
                ...
                hide-selected
                :rules="[(v) => !field.required || (field.multiple ? (v && v.length > 0) : !!v) || `${field.label} is required`]"
              >
...
/**
 * Generic CRUD data-table page powering the simple entity modules
 * (organization structure, categories, subcategories, suppliers,
 * warehouses, users, activity logs, notifications).
 *
 * Props:
 *  - load(params) → { data, meta }                    (required)
 *  - createForm / editForm: { fields, defaults }       (optional)
 *  - submit(values, editingRow) → Promise              (required with createForm)
 *  - destroy(row) → Promise                            (optional)
 *  - columns: q-table columns config                   (required)
 *  - perms: permission prefix, e.g. 'organization'     (required for gating)
 */
...
async function save() {
  ...
  try {
    ...
    $q.notify({ type: 'positive', message: editing.value ? 'Updated successfully.' : 'Created successfully.' })
    await load()
  } catch (e) {
...
function confirmDestroy(row) {
  $q.dialog({
    ...
    message: `Are you sure you want to archive this ${entityLabel}? Historical data is kept, but it will no longer appear in lists.`,
    cancel: true,
    persistent: true,
    color: 'negative',
  }).onOk(async () => {
    try {
      ...
      $q.notify({ type: 'positive', message: 'Archived successfully.' })
      await load()
    } catch (e) {
